...
namespace RegistrarServices
{
    internal class SchoolYearLogic
    {
        ...
        //this function gets the next opened school year
        public CommonExchange.SchoolYear GetNextOpenedSchoolYearInformation(CommonExchange.SysAccess userInfo, 
            CommonExchange.SchoolYear yearInfo)
        {
            CommonExchange.SchoolYear nextYearInfo = new CommonExchange.SchoolYear();

            nextYearInfo.DateStart = this.GetSchoolYearDateStart(userInfo, true).ToString();
            nextYearInfo.DateEnd = this.GetSchoolYearDateEnd(DateTime.Parse(nextYearInfo.DateStart), true).ToString();
            nextYearInfo.Description = this.GetSchoolYearDescription(DateTime.Parse(yearInfo.DateStart),
                DateTime.Parse(nextYearInfo.DateEnd), true);
            nextYearInfo.IsSummer = true;
        ...
        private DateTime GetSchoolYearDateEnd(DateTime dateStart, Boolean isSummer)
        {
            if (isSummer)
            {
                return DateTime.Parse(dateStart.AddMonths((Byte)_stdRegistrar.SummerMonths - 1).Month.ToString() + "/" +
                    DateTime.DaysInMonth(dateStart.AddMonths(1).Year, dateStart.AddMonths((Byte)_stdRegistrar.SummerMonths - 1).Month).ToString() + "/" +
                    dateStart.AddMonths((Byte)_stdRegistrar.SummerMonths - 1).Year.ToString() + " 11:59:59 PM");
            }
            else
            {
                return DateTime.Parse(dateStart.AddMonths(((Byte)_stdRegistrar.SemesterMonths * 2) - 1).Month.ToString() + "/" +
                    DateTime.DaysInMonth(dateStart.AddMonths(((Byte)_stdRegistrar.SemesterMonths * 2) - 1).Year, 
                    dateStart.AddMonths(((Byte)_stdRegistrar.SemesterMonths * 2) - 1).Month).ToString() + "/" +
                    dateStart.AddMonths(((Byte)_stdRegistrar.SemesterMonths * 2) - 1).Year.ToString() + " 11:59:59 PM");
            }

        } //------------------------------

        //this function gets the school year description
        private String GetSchoolYearDescription(DateTime dateStart, DateTime dateEnd, Boolean isSummer)
        {
            if (!isSummer)
            {
            ...
            {
                return "S.Y. " + dateStart.Year.ToString() + " - " + dateEnd.Year.ToString() + " (SUMMER)";
            }
        } //----------------------------              
