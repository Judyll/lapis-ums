...
{
    partial class SchoolFeeParticularCreate
    {
        ...
        public SchoolFeeParticularCreate()
        {
        ...
        //#####################################################CLASS SCHOOLFEEPARTICULARCREATE EVENTS #########################################################
        //event is raised when the class is Closing
        private void ClassClosing(object sender, FormClosingEventArgs e)
        {
            if (!_hasCreated)
            {
                String strMsg = "Are you sure you want to cancel the creation of a new fee particular?";
        ...
        //#####################################################END CLASS SCHOOLFEEPARTICULARCREATE EVENTS #########################################################
        ...
        private void btnCreateClick(object sender, EventArgs e)
        {
            if (this.ValidateControls())
            {
                try
                {
                    String strMsg = "Are you sure you want to create a new School Particular Fee?";

                    DialogResult msgResult = MessageBox.Show(strMsg, "Confirm Create", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);

                    if (msgResult == DialogResult.Yes)
                    {
                        strMsg = "The School Particular Fee has been successfully created.";

                        this.Cursor = Cursors.WaitCursor;

                        _schoolFeeManager.InsertSchoolFeeParticular(_userInfo, _particularInfo);

                        this.Cursor = Cursors.Arrow;

                        MessageBox.Show(strMsg, "Success", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);

                        this.txtParticularDescription.Clear();
